$(document).ready(function () {
  // alert(productObj.is_spare);

  // simple and variation radio button
  // $('.table-responsive').hide();
  // $('.product-sel').hide();

  $('#multiple-states').select2("val", [productObj.categories]);

  if (productObj.is_spare == 'spare') {
    $('.table-responsive').show();
    $('.product-sel').show();
  } else if (productObj.is_spare == 'product') {
    $('.table-responsive').hide();
    $('.product-sel').hide();
  }
    ...
    var htmlContent = `
      <tr tr_count = "${counterindex}">
        <td style="display: none;">
          <input type='text' name='id' value="" style="height: 30px !important; border-color: rgba(0, 0, 0, 0.48)" class='form-control'>
        </td>
        <td>
        ...
        <td>
          <input type='text' name='qty' value=0 style="height: 30px !important; border-color: rgba(0, 0, 0, 0.48)" class='form-control'>
        </td>
        <td>
